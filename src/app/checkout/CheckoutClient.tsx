...
export default function CheckoutClient({ userProfile }: { userProfile: any }) {
  ...
  const inputClasses = "w-full p-3 border border-gray-300 rounded-xl outline-none focus:ring-2 focus:ring-walmart-blue text-gray-900 bg-white placeholder-gray-400 font-medium"
  ...
  if (successOrders) {
    return (
      <main className="max-w-3xl mx-auto px-4 py-16 text-center space-y-8">
        <div className="w-24 h-24 bg-green-100 text-green-500 rounded-full flex items-center justify-center mx-auto text-5xl mb-6">✓</div>
        <h1 className="text-3xl font-bold text-gray-900">Commande validée !</h1>
        <p className="text-gray-500 text-lg">Merci pour votre achat. Voici vos numéros de suivi à conserver précieusement :</p>
        
        <div className="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm text-left max-w-lg mx-auto space-y-4">
          {successOrders.map((order, i) => (
            <div key={i} className="flex justify-between items-center p-4 bg-gray-50 rounded-xl border border-gray-100">
              <span className="font-medium text-gray-700">{order.shop_name}</span>
              <span className="font-mono font-bold text-walmart-blue text-lg select-all cursor-pointer" title="Double-cliquez pour copier">{order.id}</span>
            </div>
          ))}
        </div>
        
        <p className="text-sm text-gray-400 mt-4">Vous pouvez suivre ces commandes via l'onglet "Suivi rapide" dans le menu.</p>
        <Link href="/" className="inline-block mt-8 bg-walmart-blue text-white px-8 py-3 rounded-xl font-bold hover:bg-blue-700 transition-colors">Retour à la boutique</Link>
      </main>
    )
  }
    ...
    <main className="max-w-5xl mx-auto px-4 py-12">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
        ...
        <div className="space-y-8">
          ...
          <form id="checkout-form" onSubmit={handleSubmit} className="bg-white p-8 rounded-2xl border border-gray-200 shadow-sm space-y-4">
            ...
            {!userProfile && (
              <div className="mt-8 pt-6 border-t border-gray-200">
                <h3 className="text-lg font-bold text-gray-900 mb-2">Créer un compte (Optionnel)</h3>
                <p className="text-sm text-gray-500 mb-4">Gagnez du temps pour vos prochains achats et suivez vos commandes facilement en choisissant un mot de passe.</p>
                <div>
        ...
        {/* RÉSUMÉ ET PAIEMENT */}
        <div className="space-y-8">
          <h2 className="text-2xl font-bold text-gray-900">Résumé du paiement</h2>
          <div className="bg-walmart-darkBlue text-white p-8 rounded-3xl shadow-xl space-y-6">
            <div className="space-y-2">
              {cart.map((item) => (
                <div key={item.product_id} className="flex justify-between text-sm opacity-80">
            ...
            <button form="checkout-form" disabled={isSubmitting} className="w-full bg-white text-walmart-darkBlue py-4 rounded-2xl font-bold text-lg hover:bg-gray-100 transition-all shadow-lg disabled:opacity-50">
